...
@section('content')
    ...
    <main class="py-6 bg-[#E2E2E2] min-h-screen text-[#000C28]">
        <div class="mx-auto max-w-5xl px-4 sm:px-6">
            ...
            <!-- Main Layout Grid (Dua Kolom) -->
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">

                <!-- KIRI: Summary Card & Info Cards -->
                ...
                <!-- 2. Info Cards (Mobile: 3 Horizontal, Desktop: Vertikal ke Bawah) -->
                    <div class="grid grid-cols-3 lg:grid-cols-1 gap-2 sm:gap-3">
                        ...
                        <!-- Card 1: Time Limit -->
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 border-t-[3px] border-t-[#FD4801] p-2.5 flex flex-col justify-center items-center lg:flex-row lg:justify-between text-center lg:text-left hover:shadow-md transition">
                            <div class="w-full">
                                <h3 class="font-black text-[8px] sm:text-[9px] text-[#FD4801] uppercase tracking-wider mb-1 lg:mb-0.5">Validity</h3>
                                <p class="font-bold text-[10px] sm:text-xs text-[#000C28] leading-none">15 Mins</p>
                            </div>
                            <span class="hidden lg:inline-flex text-[9px] font-semibold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-md shrink-0">Timeout</span>
                        </div>

                        <!-- Card 2: Methods -->
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 border-t-[3px] border-t-[#FD4801] p-2.5 flex flex-col justify-center items-center lg:flex-row lg:justify-between text-center lg:text-left hover:shadow-md transition">
                            <div class="w-full">
                                <h3 class="font-black text-[8px] sm:text-[9px] text-[#FD4801] uppercase tracking-wider mb-1 lg:mb-0.5">Method</h3>
                                <p class="font-bold text-[10px] sm:text-xs text-[#000C28] leading-none">QRIS Only</p>
                            </div>
                            <span class="hidden lg:inline-flex text-[9px] font-semibold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md shrink-0">Exclusive</span>
                        </div>

                        <!-- Card 3: E-Ticket -->
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 border-t-[3px] border-t-[#FD4801] p-2.5 flex flex-col justify-center items-center lg:flex-row lg:justify-between text-center lg:text-left hover:shadow-md transition">
                            <div class="w-full">
                                <h3 class="font-black text-[8px] sm:text-[9px] text-[#FD4801] uppercase tracking-wider mb-1 lg:mb-0.5">Ticket</h3>
                                <p class="font-bold text-[10px] sm:text-xs text-[#000C28] leading-none">Via Email</p>
                            </div>
                            <span class="hidden lg:inline-flex text-[9px] font-semibold text-green-600 bg-green-50 px-2 py-0.5 rounded-md shrink-0">Instant</span>
                        </div>
                ...
                <div class="lg:col-span-8 flex flex-col gap-4">
                    <div class="bg-[#F5F7FA] rounded-xl shadow-sm p-4 sm:p-5 border border-gray-100">
                        ...
                        <div class="bg-white rounded-xl p-2 border border-gray-100 flex justify-center items-center overflow-hidden min-h-[380px]">
                            <div id="snap-container" class="w-full overflow-x-auto"></div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    // 2. MESIN PEMBUAT MODAL 10 DETIK (V2 - Dengan Icon & Dynamic Redirect)
    function showDynamicModal(type) {
        isLocked = false; // Buka kunci layar biar bisa pindah halaman nanti
        
        // 🔥 EKSEKUSI PENGHAPUSAN DRAFT DI SINI:
        // Berlaku buat error maupun sukses, memori form lu bakal langsung dibilas bersih.
        localStorage.removeItem('jtr_register_draft');
        
        const isSuccess = type === 'success';
        const bgColor = isSuccess ? '#10B981' : '#EF4444'; // Emerald (Sukses) / Merah (Error)
        ...
        // 🔥 LOGIKA REDIRECT: Sukses ke Dashboard ('/'), Error ke Register ('/register')
        ...
        // Gabungkan elemen ke dalam kotak
        box.innerHTML = `
            ${iconHtml}
        ...
        // Tempel modal ini secara paksa ke HTML
        overlay.appendChild(box);
        document.body.appendChild(overlay);

        // 3. LOGIKA HITUNG MUNDUR & REDIRECT DINAMIS
        let timeLeft = 10;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // 🔥 Kalau user refresh pas statusnya ternyata udah PAID di database:
        if (paymentStatus === 'paid') {
            // Langsung panggil modal sukses tanpa nunggu snap embed lagi!
