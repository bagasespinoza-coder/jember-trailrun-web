...
<header id="main-header" class="fixed w-full left-0 top-0 z-50 transition-all duration-350 {{ $isSubpage ? 'bg-white shadow-sm border-b border-gray-100 text-[#000C28]' : 'bg-transparent text-white' }} hidden lg:block" {{ $isSubpage ? 'data-navbar-static' : '' }}>
    <nav data-navbar class="relative w-full mx-auto flex h-20 items-center justify-between px-6 transition-all duration-300 lg:px-12 max-w-[1400px]">
